...
def _predict_flow_field(
    ...
    """
    solid_mask = _make_solid_mask_from_gray(
        gray_model,
    ...
    x_flow = np.stack([solid_mask, alpha_channel], axis=-1)[np.newaxis].astype('float32')
    ...
    field = y_pred[0, :, :, field_channel].astype(np.float32)
    field[solid_mask > 0.5] = np.nan
...
def create_airfoil_draw_widget(
    ...
    smooth_draw = widgets.Checkbox(value=True, description='Smooth draw', indent=False)
    ...
    def on_mouse_move(x, y):
        if not drawing['active']:
            return
        apply_pen_style()
        if smooth_draw.value and drawing['last_x'] is not None:
            lx = drawing['last_x']
            ly = drawing['last_y']
            mid_x = (lx + x) / 2.0
            mid_y = (ly + y) / 2.0
            canvas.quadratic_curve_to(lx, ly, mid_x, mid_y)
            canvas.stroke()
            drawing['last_x'] = x
            drawing['last_y'] = y
        else:
    ...
    def on_mouse_up(x, y):
        if drawing['active']:
            if smooth_draw.value and drawing['last_x'] is not None:
                canvas.quadratic_curve_to(drawing['last_x'], drawing['last_y'], x, y)
                canvas.stroke()
            else:
                canvas.line_to(x, y)
                canvas.stroke()
    ...
    def on_predict(_):
        ...
        gray_model = _resize_gray_to_model(gray)

        x_coeff = gray_model[np.newaxis, :, :, np.newaxis].astype('float32')
        mean_norm, std_norm = _mc_predict(model, x_coeff, n_mc)

        html = _make_coeff_html(mean_norm, std_norm, angle_deg)
        html += _make_flow_html(gray_model, angle_deg)
    ...
    draw_controls = widgets.HBox([smooth_draw, line_width])
