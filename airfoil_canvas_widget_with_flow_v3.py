...
def create_airfoil_draw_widget(
    ...
    # Python-side mirror of the canvas.
    #
    # ipycanvas.get_image_data() can occasionally lag behind the browser
    # drawing state, especially if Predict/Move/Rotate is clicked immediately
    # after drawing. This buffer is updated during mouse strokes and used for
    # prediction/transforms, so button actions operate on the full drawing.
    canvas_state = {
        'rgba': np.zeros((CANVAS_H, CANVAS_W, 4), dtype=np.uint8)
    }
    canvas_state['rgba'][:, :, 3] = 255
    ...
    def get_canvas_rgba():
        # Use the Python-side mirror instead of reading from the browser canvas.
        # This avoids stale/incomplete snapshots after fast button clicks.
        return canvas_state['rgba'].copy()

    def _commit_current_stroke():
        """
        Finalize any active stroke before reading canvas pixels.

        This helps when the user clicks Predict/Move/Rotate immediately after
        drawing, before the browser has fired/processed the mouse-up event.
        """
        if drawing.get('active') and drawing.get('last_x') is not None:
            apply_pen_style()
            canvas.line_to(drawing['last_x'], drawing['last_y'])
            canvas.stroke()
    ...
    def snapshot_canvas_rgba():
        """Commit any pending stroke and then read the current canvas pixels."""
        _commit_current_stroke()
        return get_canvas_rgba()
    ...
    def _draw_state_dot(x, y):
        width = int(line_width.value)
        radius = max(1, width // 2)
        img = Image.fromarray(canvas_state['rgba'], mode='RGBA')
        ...
        canvas_state['rgba'] = np.array(img, dtype=np.uint8)

    def _draw_state_line(x0, y0, x1, y1):
        width = int(line_width.value)
        radius = max(1, width // 2)
        img = Image.fromarray(canvas_state['rgba'], mode='RGBA')
        draw = ImageDraw.Draw(img)
        ...
        # Round caps to match the visual canvas stroke.
        draw.ellipse(
    ...
    def on_mouse_move(x, y):
        ...
        if drawing['last_x'] is not None:
            _draw_state_line(drawing['last_x'], drawing['last_y'], x, y)
        else:
            _draw_state_dot(x, y)
